...
# Add local mamba module to path
_mamba_dir = os.path.join(os.path.dirname(__file__), '../../mamba')
if os.path.isdir(_mamba_dir) and _mamba_dir not in sys.path:
    sys.path.insert(0, _mamba_dir)

# Add local causal_conv1d module to path
_causal_conv1d_dir = os.path.join(os.path.dirname(__file__), '../../causal-conv1d')
if os.path.isdir(_causal_conv1d_dir) and _causal_conv1d_dir not in sys.path:
    sys.path.insert(0, _causal_conv1d_dir)
...
# Provide helpful error if imports failed
if selective_scan_fn is None:
    def _raise_selective_scan_error(*args, **kwargs):
        raise ImportError(
            "selective_scan_fn is not available. "
            "Please install mamba-ssm: pip install mamba-ssm"
        )
    selective_scan_fn = _raise_selective_scan_error
